...
| `make-icon.sh` | Regenerates `AppIcon.icns` + `dmg-logo.png` from the full-bleed `AppIcon-source-v7.png` artwork. The renderer clips the gradient to one full-canvas macOS rounded square with transparent outer corners; the folder keeps its breathing room and the approved large 深圳 lettering. |
