...
| `make-icon.sh` | Regenerates `AppIcon.icns` + `dmg-logo.png` from the full-bleed `AppIcon-source-v6.png` artwork. Its square white-to-icy-blue gradient lets macOS apply one native squircle without shrinking a transparent foreground into a nested icon. |
