...
| `make-icon.sh` | Regenerates `AppIcon.icns` + `dmg-logo.png` from the approved `AppIcon-source-v3.png` master through `make-logo.swift`'s deterministic, full-bleed macOS squircle mask. The small white 深圳 signature stays fully inside the folder at every icon size. |
