...
| `make-icon.sh` | Regenerates the modern `Assets.car` icon catalog, legacy `AppIcon.icns` fallback, and `dmg-logo.png` from the approved `AppIcon-source-v3.png` master. The small white 深圳 signature stays fully inside the folder at every icon size. Requires Xcode's `actool`. |
