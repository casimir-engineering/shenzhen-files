...
| `make-icon.sh` | Regenerates `AppIcon.icns` + `dmg-logo.png` from the full-bleed `AppIcon-source-v7.png` artwork. Its square white-to-icy-blue gradient lets macOS apply one native squircle; the smaller folder restores breathing room while keeping the 深圳 lettering at its prior absolute size. |
