...
**Date:** 2026-09-12 · **Version:** 26.9.12 build 1 (upstream 51.beta) · arm64,
...
| Update check verified | Direct signed-helper E2E: the fixed helper replaced a disposable `26.7.22-1` bundle with exact tag `26.8.27-2` / global bundle build `26082702`, then relaunched from the canonical new bundle path; final executable and `AppIcon.icns` hashes matched the staged payload and the secure staging directory was removed. A signed payload deliberately paired with the wrong expected tag was rejected before readiness, leaving the target unchanged. Published `26.7.22-1` and `26.8.27-1` installations still execute their own old helper during replacement, so they require one manual DMG replacement before later automatic updates can use the repaired helper. |
