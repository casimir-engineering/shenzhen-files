...
| Mounted volumes | `src/nautilus-macos-volumes.m` supplies the startup disk, external disks, disk images, and network volumes missing from GLib's Darwin volume monitor. The sidebar merges them into the mounts section without GIO duplicates, watches `/Volumes` for hot-plug refresh, and provides Finder-like eject/unmount controls through `NSWorkspace`. |
