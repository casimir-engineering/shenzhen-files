...
| Logo / icon | `package/AppIcon-source-v7.png` is the approved folder-first master with the large, legible white **深圳** signature fully contained inside the folder. `make-logo.swift` clips it to one full-canvas 22.5%-radius macOS rounded square with genuinely transparent outer corners → `AppIcon.icns` + `dmg-logo.png`; the bundled startup path explicitly loads that icon so an in-place update refreshes the Dock immediately. |
