...
**Date:** 2026-09-14 · **Version:** 26.9.12 build 1 (upstream 51.beta) · arm64,
...
| Updater incident + corrected validation | **Published builds through `26.9.12-1` are affected.** The production Foundation `NSTask` launch makes the helper a process-group leader, so its unconditional `setsid()` returned `EPERM` and produced “The update installer exited before it was ready.” The earlier direct-shell helper test did not reproduce `NSTask` semantics and was not end-to-end. Source now accepts only the expected `EPERM` case where `getpgrp() == getpid()`. The signed local candidate passed `package/test-updater-helper-e2e.sh`: Foundation readiness marker, move-aside swap, exact-path relaunch, tag `26.9.12-1`, build `26091201`, executable hash, and `AppIcon.icns` hash. This candidate-helper test is enforced by the signing script, and repository rules separately require a real previous-public-release download/DMG update before publication. Users of affected builds need one manual fixed-DMG bootstrap because the failing code is in the already-installed helper. |
