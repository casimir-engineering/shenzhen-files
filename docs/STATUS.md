...
| Logo / icon | `package/AppIcon-source-v3.png` is the approved folder-first ImageGen master on a polished macOS squircle. The small white **深圳** signature is fully contained inside the folder; `make-logo.swift` applies deterministic safe bounds per size → `AppIcon.icns` + `dmg-logo.png`; the DMG background uses the same mark. |
