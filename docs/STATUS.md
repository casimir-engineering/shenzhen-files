...
| Updater incident + corrected validation | **Published builds through `26.9.12-1` are affected.** The production Foundation `NSTask` launch makes the helper a process-group leader, so its unconditional `setsid()` returned `EPERM` and produced “The update installer exited before it was ready.” The earlier direct-shell helper test did not reproduce `NSTask` semantics and was not end-to-end. `26.9.14-1` accepts only the expected `EPERM` case where `getpgrp() == getpid()`. The release pipeline now runs `package/test-updater-helper-e2e.sh` before notarization and against the quarantined app copied from the notarized DMG, checking the Foundation readiness marker, move-aside swap, exact-path relaunch, release tag, bundle build, executable hash, and `AppIcon.icns` hash. Users of affected builds need one manual `26.9.14-1` bootstrap because the failing code is in the already-installed helper; later updates use the corrected helper. |
