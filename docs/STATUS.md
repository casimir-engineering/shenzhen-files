...
| Logo / icon | `package/make-logo.swift` renders a folder-first mark in Shenzhen blue, with a much smaller **深圳** signature so the folder remains unmistakable at Dock size; `make-icon.sh` renders the full iconset natively per size → `AppIcon.icns` + `dmg-logo.png`; the DMG background uses the same mark |
